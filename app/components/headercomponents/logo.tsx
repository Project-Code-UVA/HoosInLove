// app/components/headercomponents/logo.tsx
import React from 'react';
import { StyleSheet, Text, View } from 'react-native';
import Svg, { Path, Text as SvgText } from 'react-native-svg';

type Props = {
  scale?: number; // controls overall size
};

export default function Logo({ scale = 1 }: Props) {
  const HEART_W = 63 * scale;
  const HEART_H = 49 * scale;

  const LOVE_W = 120 * scale;
  const LOVE_H = 60 * scale;

  const FONT_SIZE_TOP = 18 * scale;
  const LOVE_FONT_SIZE = 48 * scale;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* LOVE */}
        <Svg width={LOVE_W} height={LOVE_H} style={styles.loveSvg}>
          <SvgText
            x={LOVE_W / 2}
            y={LOVE_H * 0.7}
            fontSize={LOVE_FONT_SIZE}
            fontFamily="GwendolynBold"
            textAnchor="middle"
            fill="none"
            stroke="#002562"
            strokeWidth={4 * scale}
